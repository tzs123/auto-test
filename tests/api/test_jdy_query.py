...
@allure.feature("今东车融-查询接口")
@pytest.mark.api
@pytest.mark.parametrize("case", load_cases("cases/api/test_jdy_query.yaml"))
def test_jdy_query(case):
    with allure.step(case["name"]):
        ...
        resp = requests.get(url, headers=headers, params=case.get("params"),
